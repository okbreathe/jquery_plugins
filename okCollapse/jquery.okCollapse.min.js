...
return this.each(function(){var self;$(opts.toggleSelector).live(opts.toggleEvent,dispatch);$(this).find("ul").each(function(){self=$(this);if(self.children().length<=1){self.siblings(opts.toggleSelector+":first").hide();}else{self.data("maxHeight",self.height()).css({height:0,opacity:0}).addClass(opts.collapsedClass);}});});};})(jQuery);