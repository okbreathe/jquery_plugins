...
$(this).animate({top:'-='+settings.distance+'px',opacity:isHidden?1:0},settings.effectTime,function(){if(callback){callback(this);}});});};$.fn.okTips=function(opts){var popup,cover,trigger;opts=$.extend({template:"<div id='ui-oktip' class='ui-tooltip'></div>",live:false,triggerEvent:'click',closeTrigger:null,triggerCloses:true,beforeShow:null,afterShow:null,beforeDestroy:null,afterDestroy:null,body:function(trigger){return"<p>"+trigger.title+"</p>";},top:-20,left:20,hideDelay:500,effectTime:250,distance:10,modal:true,overlayClass:'ui-widget-overlay'},opts);function appendHtml(){var html;if(popup.length===0){popup=$(opts.template).appendTo(document.body).hide();if(opts.modal){cover=$("<div id='ui-oktip-overlay'></div>");cover.addClass(opts.overlayClass).css({position:'absolute',top:0,left:0,width:"100%",height:$("body").height()+'px',zIndex:"1000"}).appendTo(document.body).hide();}}
popup.bind('destroy',destroy);html=typeof(opts.body)=='function'?opts.body.call(popup,trigger):opts.body;if(typeof(html)==="string"){popup.html(html);}}
...
function create(e){var visible,pos,html;appendHtml();visible=popup.is(":visible");pos=getPosition(e);popup.stop(true).css({position:'absolute',zIndex:'9999',left:pos.left,top:pos.top});bindDestroyEvent();if(opts.beforeShow){opts.beforeShow.call(popup,trigger);}
...
return popup;}
function destroy(e){if(opts.beforeDestroy){opts.beforeDestroy.call(popup,trigger);}
